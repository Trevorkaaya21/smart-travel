...
type Diary = { id: string; title: string; created_at: string; trip_id?: string | null; trip_name?: string | null }
...
export default function DiaryPage() {
  ...
  const preselectedTripId = searchParams.get('tripId') ?? undefined
  const preselectedTripName = searchParams.get('tripName') ?? undefined

  const [title, setTitle] = React.useState(preselectedTripName ? `${preselectedTripName} Diary` : 'My Travel Diary')
  const [selectedTripId, setSelectedTripId] = React.useState<string>(preselectedTripId ?? '')
  ...
  const createMut = useMutation({
    mutationFn: () => createDiary(title.trim() || 'Untitled entry', email!, selectedTripId || undefined),
    onSuccess: (data) => {
      ...
      setSelectedTripId('')
      if (data?.id) {
        window.location.href = `/dashboard/diary/${data.id}`
      }
    },
      ...
      <header className="content-header">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-2">
            ...
            <h1 className="text-3xl font-semibold md:text-4xl">Your travel stories</h1>
            <p className="max-w-2xl text-sm text-[color-mix(in_oklab,rgb(var(--text))_72%,rgb(var(--muted))_28%)]">
              Write about your adventures, attach photos, and link each diary to a trip so you can relive the journey.
            </p>
          </div>
          <div
      ...
      <section className="content-card space-y-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="space-y-2">
            ...
            <p className="form-helper uppercase tracking-[0.3em]">Name it and optionally link to a trip</p>
          </div>
          <div className="content-subtle flex w-full flex-col gap-3 p-4 md:w-auto md:flex-row md:items-center md:flex-wrap">
            <Input
              value={title}
            ...
            <select
              ...
              className="input-surface h-10 rounded-xl border px-3 text-sm md:w-48"
              style={{ borderColor: 'var(--glass-border)', background: 'var(--glass-bg)', color: 'rgb(var(--text))' }}
            >
              <option value="">No trip linked</option>
              {trips.map((t) => (
                <option key={t.id} value={t.id}>{cleanTripName(t.name)}</option>
              ))}
            </select>
            <Button
        ...
        {diariesQuery.isLoading ? (
          ...
          <div className="content-card flex flex-col items-center justify-center gap-4 text-center text-[color-mix(in_oklab,rgb(var(--text))_70%,rgb(var(--muted))_30%)]">
            ...
            <p className="max-w-sm text-sm">
              You haven&apos;t written any entries yet. Create one above to start capturing your travel memories.
            </p>
          </div>
        ) : (
          <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
            {diaries.map((diary) => (
              <Link
                ...
                className="content-card flex h-full flex-col justify-between gap-4 text-sm transition hover:-translate-y-[2px]"
              >
                <div className="space-y-3">
                  <span className="form-label">Diary entry</span>
                  <h3 className="text-lg font-semibold text-[rgb(var(--text))]">{diary.title}</h3>
                  {diary.trip_id && (
                    <span className="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium text-[rgb(var(--accent))]"
                      style={{ borderColor: 'rgba(var(--accent) / 0.3)', background: 'rgba(var(--accent) / 0.08)' }}
                    >
                      <MapPinned className="h-3 w-3" />
                      {diary.trip_name ? cleanTripName(diary.trip_name) : 'Linked to trip'}
                    </span>
                  )}
                </div>
