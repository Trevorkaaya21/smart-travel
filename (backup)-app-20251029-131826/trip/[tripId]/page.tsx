...
function DayColumn(props: { day: number; children: React.ReactNode }) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`rounded-2xl border border-slate-300 dark:border-white/15 bg-white/60 dark:bg-white/5 p-3 ${isOver ? 'ring-2 ring-white/30' : ''}`}
    >
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <select
          ...
          className="rounded-md border border-slate-300 dark:border-white/15 bg-transparent px-2 py-1 text-xs"
          title="Move to day"
        >
        ...
        <input
          ...
          className="w-40 rounded-md border border-slate-300 dark:border-white/15 bg-transparent px-2 py-1 text-xs"
        />
        <button
...
export default function TripPage() {
  ...
  const itemsQ = useQuery({ queryKey: ['trip-items', tripId], queryFn: () => fetchItems(tripId), enabled: !!tripId })
            ...
            return (
              <DayColumn key={day} day={day}>
                <SortableContext items={ids} strategy={verticalListSortingStrategy}>
                  {list.length === 0 && (
                    <div className="rounded-xl border border-dashed border-slate-300 dark:border-white/15 p-4 text-center text-sm opacity-60">
